...
export interface LoginCredentials { phone: string; email: string; password: string; remember: boolean }
export function Login({ onLogin, onRegister }: { onLogin: (credentials: LoginCredentials) => Promise<void>; onRegister: () => void }) {
  const [remember, setRemember] = useState(false);
  const [phone, setPhone] = useState(''); const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!phone.trim() && !email.trim()) { setError('Masukkan email atau nomor telepon.'); return; } setLoading(true); setError(''); try { await onLogin({ phone, email, password, remember }); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Login gagal.'); } finally { setLoading(false); } };
  return <AuthShell><AuthTabs active="login" onLogin={() => undefined} onRegister={onRegister} /><form className="auth-form" onSubmit={submit}><TextField label="Nomor Telepon" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08123xxxxx" type="tel" inputMode="tel" pattern="[0-9+ -]{8,15}" /><TextField label="Email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="abcd@gmail.com" type="email" autoComplete="email" /><TextField label="Password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Masukkan password" type="password" minLength={8} autoComplete="current-password" required /><div className="form-options"><Checkbox checked={remember} onChange={setRemember} label="Ingat saya" /><button type="button" className="text-button">Lupa password?</button></div>{error&&<p className="form-error" role="alert">{error}</p>}<button className="primary-button" type="submit" disabled={loading}>{loading?'Memproses...':'Masuk'}</button></form></AuthShell>;
}
...
interface ProfileProps { draft: SignupDraft; onDraftChange: (draft: SignupDraft) => void; onBack: () => void; onComplete: (draft: SignupDraft) => Promise<void> }
export function ProfileForm({ draft, onDraftChange, onComplete }: ProfileProps) {
  const [error,setError]=useState(''); const [loading,setLoading]=useState(false);
  const update = (field: keyof SignupDraft, value: string) => onDraftChange({ ...draft, [field]: value });
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const clean = { ...draft, name: draft.name.trim(), location: draft.location.trim(), email: draft.email.trim() }; if (clean.name.length < 2 || clean.location.length < 3 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(clean.email) || clean.password.length < 8) return; setLoading(true); setError(''); try { onDraftChange(clean); await onComplete(clean); } catch(reason) { setError(reason instanceof Error?reason.message:'Pendaftaran gagal.'); } finally { setLoading(false); } };
  return <AuthShell><form className="flow-form profile-flow" onSubmit={submit}><h1>Lengkapi profil</h1><div className="profile-fields"><TextField label="Nama" value={draft.name} onChange={(e) => update('name', e.target.value)} placeholder="Ayubi Fathan" autoComplete="name" minLength={2} required /><TextField label="Lokasi" value={draft.location} onChange={(e) => update('location', e.target.value)} placeholder="Provinsi, Daerah" minLength={3} required /><TextField label="Email" value={draft.email} onChange={(e) => update('email', e.target.value)} placeholder="abcd@gmail.com" type="email" autoComplete="email" required /><TextField label="Password" value={draft.password} onChange={(e) => update('password', e.target.value)} placeholder="Minimal 8 karakter" type="password" minLength={8} autoComplete="new-password" required /></div>{error&&<p className="form-error" role="alert">{error}</p>}<button className="primary-button" type="submit" disabled={loading}>{loading?'Menyimpan...':'Simpan'}</button></form></AuthShell>;
}
